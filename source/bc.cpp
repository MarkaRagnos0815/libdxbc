...
inline uint16_t Encode565(_In_ const HDRColorA *pColor) noexcept {
    ...
    w = static_cast<uint16_t>((static_cast<int32_t>(Color.r * 31.0f + 0.5f) << 11) | (static_cast<int32_t>(Color.g * 63.0f + 0.5f) << 5) | (static_cast<int32_t>(Color.b * 31.0f + 0.5f) << 0));

    return w;
}

//-------------------------------------------------------------------------------------
void OptimizeRGB(_Out_ HDRColorA *pX, _Out_ HDRColorA *pY, _In_reads_(NUM_PIXELS_PER_BLOCK) const HDRColorA *pPoints, uint32_t cSteps, uint32_t flags) noexcept {
